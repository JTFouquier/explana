...
**``r iterations` iterations`** were performed. BorutaSHAP was used with **``r borutashap_trials` trials`** to find features that perform repeatedly better than shuffled versions of all input features. Selected features were considered important if they performed better than **``r borutashap_threshold`%`** of the importance score of the best shuffled feature with a false discovery rate of **``r borutashap_p`%`**.

These methods were copy/pasted from an EXPLANA (version: ``r version``) feature selection report created on ``r Sys.Date()`` (see github.com for code and additional documentation; Copyright 2023 Regents of the University of Colorado. All rights reserved. Authored by Jennifer Fouquier).
