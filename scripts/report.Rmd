...
EXPLANA assists with selecting which features, out of many input features, that are best related to a response variable by using well-known machine-learning algorithms combined with novel methods.

Input features and response variables can be numerical, categorical, or from non-normal data distributions. The relationships between selected features and the response can be linear, or complex, non-linear relationships. A report is generated that textually and graphically summarizes results for streamlined exploratory analysis and hypothesis generation.
...
**``r iterations``** iterations were performed. BorutaSHAP was used to find features that perform repeatedly better than shuffled versions of all input features. Features were considered important if they performed better than **``r borutashap_threshold``****%** of the importance score of the best shuffled feature using **``r borutashap_trials``** trials, p=**``r borutashap_p``**. Categorical variables were binary encoded and **``r enc_percent_threshold_print``**.

These methods are taken from an EXPLANA feature selection report (version: **``r version``**) created on **``r Sys.Date()``**. See github.com for code and additional documentation. Copyright 2023 Regents of the University of Colorado. All rights reserved.
