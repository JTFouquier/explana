...
original_path <- paste0(out_path, snakemake@config[["path_original"]])
first_path <- paste0(out_path, snakemake@config[["path_first"]])
previous_path <- paste0(out_path, snakemake@config[["path_previous"]])
pairwise_path <- paste0(out_path, snakemake@config[["path_pairwise"]])
...
pretty_table <- function(dataset_type, out_path, display_file) {
  ...
  df_path <- paste0(out_path,
  snakemake@config[[paste0("path_", dataset_type)]],
...
EXPLANA (EXploratory ANAlysis) Methods:

EXPLANA assists with understanding the relationship between a response variable and original feature values. For longitudinal datasets, changes/deltas in feature values are automatically computed. Delta datasets (First, Previous and Pairwise delta datasets) are calculated using different reference points. This is important because features can carry varying degrees of importance between models built from different delta datasets. This workflow helps identify differences in feature relationships to a response variable by providing feature selection results from multiple models.

Please ensure you understand the workflow and that the percent 
variation explained (using out-of-bag/OOB scores) for each model is adequate for your purposes. When you are using all your data without prior hypotheses, you are performing *exploratory analysis* and should make this clear
...
**Features selected as important from models built with Original and First, Previous and Pairwise delta datasets.** Average impact on response is displayed for positive instances of encoded variables. For numerical variables, the relationship to the response variable is more difficult to calculate, and indicated with "+/-". Grey boxes indicate features included in the model for each dataset. For comprehensive list of input features, see provided output. Random Forests can identify complex interactions and non-linear relationships. Post-hoc tests and further research is important.
