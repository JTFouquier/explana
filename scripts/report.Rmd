...
package_list <- c("knitr", "htmltools", "tidyverse", "Hmisc", "jsonlite")
...
make_tab_input <- function(dataset_type, out_path) {
  input_vars <- paste0(out_path,
  snakemake@config[[paste0("path_rf_", dataset_type)]],
  paste0(dataset_type, "-input-features.txt"))
  cat(readLines(input_vars), sep = "\n")
}
...
if (enc_percent_threshold == "0") {
  ...
  enc_percent_threshold_print <- "low occuring or rare categorical variables were not removed"
...
make_tab_input("original", out_path = out_path)
```
...
```{r out.width = 1000, out.height = 600, comment=""}
make_tab_input("previous", out_path)
```
...
```{r out.width = 1000, out.height = 600, comment=""}
make_tab_input("pairwise", out_path)
```
