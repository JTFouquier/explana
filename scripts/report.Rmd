---
title: "EXPLANA: Feature Selection Report"
...
# SHAP figure text
rmd_text <- c(
  "**SHAP summary beeswarm plots of top selected features.** Plots show top features' influence on the machine learning prediction of a response variable. Each point represents one sample, and the horizontal position indicates impact on the response as indicated on the x-axis. Points to the left indicate a negative impact, and points to the right indicate a positive impact. The colors represent feature values, where red is larger and blue is smaller. For binary encoded features red is yes[1] and blue is no[0]. SHAP is an improvement upon other importance scores because it provides information about both rank (how helpful the feature was compared to other features [y-axis]) and impact (a positive or negative impact on response values [x-axis]). If multiple figures are shown, scales may vary. A maximum of ten features are shown per plot."
)
...
You are permitted to copy the following text verbatim for use in methods to ensure reproducibility. 

EXPLANA was used for exploratory analysis to identify important features 
...
**``r iterations` iterations`** were performed. BorutaSHAP was used with 
**``r borutashap_trials` trials`** to find features that perform repeatedly better than shuffled versions of all input features. Selected features were considered important if they performed better than
**``r borutashap_threshold`%`** of the importance score of the best shuffled feature with a false discovery rate of **``r borutashap_p`%`**.
...
**Selected features from models built with Original and, for longitudinal analyses, First, Previous and Pairwise delta datasets.** Average impact on response is displayed for positive instances of encoded variables. For numerical variables, the relationship to the response variable is more complex to determine, and indicated with "+/-". Grey boxes indicate features included in the model for each dataset. For comprehensive list of input features, see provided output in sections below. Random Forests can identify complex interactions and non-linear relationships, therefore post-hoc tests and further research is important.
